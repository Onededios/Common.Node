...
describe('Parser', () => {
	...
	describe('parseAsEnum()', () => {
		...
		it('should return the value when it is in the allowed list', () => {
			allowed.forEach((x) => expect(Parser.parseAsEnum(x, allowed)).toBe(x));
		});
	...
	describe('parseAsGUID()', () => {
		const allowed = ['f81d4fae-7dec-11d0-a765-00a0c91e6bf6'];

		it('should return the value when it is in the allowed list', () => {
			allowed.forEach((x) => expect(typeof Parser.parseAsGUID(x)).toBe('string'));
		});

		it('should throw an error when the value is not allowed', () => {
			expect(() => Parser.parseAsGUID('yellow')).toThrowError('Invalid GUID');
		});
	});
});
